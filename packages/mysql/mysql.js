...
mp.events.add("playerExitVehicle", (player, vehicle) => {
	gm.mysql.handle.query('UPDATE `vehicles` SET x = ?, y = ?, z = ? WHERE id = ?', [vehicle.position.x, vehicle.position.y, vehicle.position.z, vehicle.id], function(err, res){
                                    if(!err){
                                        console.log("\x1b[92m" + vehicle.id + "\x1b[39m Vehicle updated.");
                                    } else {
                                        console.log("\x1b[31m[ERROR] " + err)
                                    }
                                });
...
//mp.events.add("saveVehicle", (entity) => {
//	if(entity.type == "vehicle"){
//		gm.mysql.handle.query('INSERT INTO `vehicles` SET id = ?, model = ?', [entity.id, entity.model], function(err, res){
//            if(!err){
